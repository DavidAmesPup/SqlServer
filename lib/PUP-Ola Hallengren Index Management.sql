/*
	This contains Ola Hallengren's maintenance scripts from:

	https://ola.hallengren.com/downloads.html

	Specificly; IndexOptimize, CommandExecute and CommandLog tables.

	To install the above tools, run this script. It will change your current DB context to Master (which is where everything is installed)

*/
USE MASTER

IF object_Id('CommandLog') IS NULL
BEGIN
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'CommandExecute')
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[CommandExecute] AS SET NOCOUNT ON'
GO
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'IndexOptimize')
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[IndexOptimize] AS SET NOCOUNT ON'
GO
